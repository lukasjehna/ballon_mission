...
def read_sensor_data(address=DEFAULT_ADDRESS):
    ...
    return {
        'timestamp': datetime.now().strftime('%H:%M:%S'),  # ms precision
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Read MPU6050 and log or print sensor data.")
    parser.add_argument(
        "--print-only",
        action="store_true",
        help="Only print readings; do not write CSV."
    )
    args = parser.parse_args()

    init_sensor()
    if args.print_only:
        print("Printing only (no CSV save).")
    else:
        print(f"Logging data to: {DATA_DIR / f'{ts}{filename}'}")

    try:
        header = ["time_HH:MM:SS", "gyro_°/s (x,y,z)", "accel_(m/s/g) (x,y,z)", "rot_° (x,y)"]
        print(header)
        while True:
            data = read_sensor_data()
            if not args.print_only:
                save_data(data, ts, filename)
            print(f"[{data['timestamp']}] "
                  f"| ({data['gyroscope']['x']:.2f}, {data['gyroscope']['y']:.2f}, {data['gyroscope']['z']:.2f}) | "
                  f" ({data['accelerometer']['x']:.3f}, {data['accelerometer']['y']:.3f}, {data['accelerometer']['z']:.3f}) | "
                  f"{data['rotation']['x']:.1f}, {data['rotation']['y']:.1f}")
            time.sleep(2)
