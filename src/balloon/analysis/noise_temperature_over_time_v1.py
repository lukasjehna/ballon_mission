...
Example:
    python3 noise_temperature_over_time.py
    python3 noise_temperature_over_time.py /path/to/measurement
    python3 noise_temperature_over_time.py --if-frequency 270 --thot 300 --tcold 5
"""
...
def noise_temperature_at_bin(
    ...
    hot_power = float(hot_spectrum[bin_index])
    cold_power = float(cold_spectrum[bin_index])
    if cold_power <= 0:
        return float("nan")
...
def calculate_noise_temperatures(
    ...
    for pair in pairs:
        ...
        if not 0 <= bin_index < hot.size:
            raise ValueError(f"Bin index {bin_index} is outside 0..{hot.size - 1}.")
        ...
        noise_temperature = noise_temperature_at_bin(
            hot, cold, bin_index, hot_temperature_k, cold_temperature_k
        )
...
def main() -> None:
    ...
    parser.add_argument("--thot", type=float, default=300.0, help="Hot temperature in K.")
    parser.add_argument("--tcold", type=float, default=5.0, help="Cold temperature in K.")
    parser.add_argument(
    ...
    if args.bin is None:
        bin_index = frequency_to_bin(args.if_frequency, bandwidth_ghz, n_bins)
    else:
        bin_index = args.bin

    times_h, temperatures_k = calculate_noise_temperatures(
        pairs, bin_index, float(thot), float(tcold)
    )
    ...
    ax.set_title(
        f"Noise temperature at IF={args.if_frequency:g} MHz (bin {bin_index}) | "
    ...
    print(f"Using spectral bin {bin_index}.")
