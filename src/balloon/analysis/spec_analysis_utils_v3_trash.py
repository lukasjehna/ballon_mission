...
def build_x_axis(n_bins: int, header_meta: Dict[str, str], x_axis_mode: str, bin_size: int = 1) -> Tuple[np.ndarray, str]:
    """Build x-axis for spectra, optionally with spectral binning."""
    if x_axis_mode == "bins":
        ...
        if bw_ghz is None or bw_ghz <= 0:
            ...
            x = np.linspace(0.0, bw_ghz, n_bins, endpoint=False)
            label = "f_IF [GHz]"

    if bin_size is None or int(bin_size) <= 1:
        return x, label

    x_binned, _ = spectral_bin_average(x, x, bin_size)
    return x_binned, label
...
def spectral_bin_average(x: np.ndarray, bin_size: int) -> Tuple[np.ndarray, np.ndarray]:
    """Return spectrally binned x/y arrays by averaging over consecutive points.

    Parameters
    ----------
    x : np.ndarray
        1D array of x-axis values (e.g. frequency or bin index).
    bin_size : int
    ...
    -------
    x_binned : np.ndarray
        Binned x values (simple mean within each group).

    Notes
    -----
    - If bin_size <= 1 or arrays are shorter than bin_size, the inputs are
      returned unchanged.
    - Any trailing samples that do not fit into a full bin are discarded.
    - NaNs are ignored in the mean; if a bin contains only NaNs, the result
      for that bin is NaN.
    """
    x = np.asarray(x, dtype=float)
    n = x.size
    b = int(bin_size)
    if b <= 1 or n == 0 or n < b:
        return x[:n].copy()
    ...
    x_reshaped = x_trim.reshape(-1, b)

    x_binned = np.nanmean(x_reshaped, axis=1)
    return x_binned