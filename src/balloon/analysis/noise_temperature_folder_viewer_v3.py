...
This variant can average over a user-defined number of hot/cold pairs before
computing the hot/cold spectra, Y-factor, and noise temperature.

Examples
--------
python3 noise_temperature_folder_viewer_generalized.py /data/run
python3 noise_temperature_folder_viewer_generalized.py /data/run --thot 295 --tcold 77
python3 noise_temperature_folder_viewer_generalized.py /data/run --pair-tolerance 60 --x-axis bins
python3 noise_temperature_folder_viewer_generalized.py /data/run --pairs-per-average 4
...
class NoiseTemperatureViewer:
    def __init__(self, pairs, header_meta, thot, tcold, x_axis, cache_size, pairs_per_average):
        if not pairs:
    ...
    def draw(self):
        try:
            ...
            self.ax_nt.set_title(
                ...
                f"T_hot={self.thot:.2f} K | T_cold={self.tcold:.2f} K"
            )
