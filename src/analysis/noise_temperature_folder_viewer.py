...
Examples
--------
python3 noise_temperature_folder_viewer_generalized.py /data/run
python3 src/analysis/noise_temperature_folder_viewer.py --thot 295 --tcold 77
python3 src/analysis/noise_temperature_folder_viewer.py --pair-tolerance 60 --x-axis bins
python3 src/analysis/noise_temperature_folder_viewer.py --thot 300 --tcold 5 --pairs-per-average 50 --spectral-bin-size 3
...
class NoiseTemperatureViewer:
    def __init__(self, pairs, header_meta, thot, tcold, x_axis, cache_size, pairs_per_average, spectral_bin_size):
        if not pairs:
    ...
    def draw(self):
        try:
            ...
            self.ax_nt.set_title(
                ...
                f"T_hot={self.thot:.2f} K | T_cold={self.tcold:.2f} K | spectral_bin={self.spectral_bin_size}"
            )
...
def main():
    ...
    parser.add_argument("--spectral-bin-size", type=int, default=1,
                        help="Average over this many adjacent spectral bins on the x-axis (default: 1, no binning).")
    ...
    if args.spectral_bin_size > 1:
        print(f"Binning x-axis over {args.spectral_bin_size} spectral bins.")
