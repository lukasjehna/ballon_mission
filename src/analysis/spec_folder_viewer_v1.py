...
class SpecViewer:
    def __init__(
        ...
        self.ax_all.grid(True, alpha=0.3)
    ...
    def build_widgets(self) -> None:
        ax_first = self.fig.add_axes([0.08, 0.05, 0.08, 0.06])
        ax_prev = self.fig.add_axes([0.17, 0.05, 0.10, 0.06])
        ax_next = self.fig.add_axes([0.28, 0.05, 0.10, 0.06])
        ax_last = self.fig.add_axes([0.39, 0.05, 0.08, 0.06])
        ax_goto = self.fig.add_axes([0.62, 0.05, 0.12, 0.06])
    ...
    def draw(self) -> None:
        ...
        nspectra = meta.get("nspectra") if isinstance(meta, dict) else None
