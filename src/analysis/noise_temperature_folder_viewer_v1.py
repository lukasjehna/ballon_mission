...
def parse_file(path: Path) -> Optional[tuple[datetime, str, Path]]:
    """Return timestamp, load type and path, or None for unrelated .spec files."""
...
def find_pairs(directory: Path, recursive: bool, tolerance_s: float) -> list[tuple[Path, Path, float]]:
    """Pair every hot file with the nearest unused cold file within tolerance."""
...
class NoiseTemperatureViewer:
    def __init__(self, pairs, header_meta, thot, tcold, x_axis, cache_size):
        ...
        (self.line,) = self.ax.plot([], [], color="tab:green", lw=1.0)
        self.ax.set_ylabel("Receiver noise temperature [K]")
    ...
    def draw(self):
        ...
        try:
            tnoise = self._load_cached(str(hot), str(cold))
            x, xlabel = sau._build_x_axis(tnoise.size, self.header_meta, self.x_axis)
            self.line.set_data(x, tnoise)
            if x.size > 1:
            ...
            self.ax.set_title(
                ...
                f"median={median:.1f} K")
        except Exception as exc:
            self.line.set_data([], [])
            self.ax.set_title(f"[{self.index + 1}/{len(self.pairs)}] ERROR: {exc}", color="tab:red")
